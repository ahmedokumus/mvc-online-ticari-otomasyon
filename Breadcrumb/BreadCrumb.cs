...
public class BreadCrumb
{
    ...
    public static string Display(string cssClassOverride = "breadcrumb float-sm-right")
    {
        ...
        state.Crumbs.Select(x => new { Entry = x, IsCurrent = IsCurrentPage(x.Key) }).OrderBy(x => x.IsCurrent).ToList().ForEach(x =>
        {
            ...
            if (x.IsCurrent)
            {
                sb.Append("<li class='breadcrumb-item active'>" + label + "</li>");
            }
            else
            {
                sb.Append("<li class='breadcrumb-item'><a href=\"" + x.Entry.Url + "\">" + label + "</a></li>");
            }
        });
        sb.Append("</ol>");
        return sb.ToString();
